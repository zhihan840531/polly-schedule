...
(function(){
  ...
  function enhanceClassDetail(classId){
    ...
    table.querySelectorAll('tbody tr').forEach((row,i)=>{const s=ensureDefaults(details[i]);if(!s||row.querySelector('.phone-cell'))return;const cells=row.querySelectorAll('td'),phoneTd=document.createElement('td');phoneTd.className='phone-cell';phoneTd.innerHTML=s.parentPhone?`<a class="student-phone-link" href="tel:${String(s.parentPhone).replace(/[^0-9+]/g,'')}" onclick="event.stopPropagation()">${s.parentPhone}</a>`:'—';const checkTd=document.createElement('td');checkTd.className='check-cell';const done=completedCount(s),btn=document.createElement('button');btn.className='btn student-check-btn'+(done===5?' done':'');btn.type='button';btn.textContent=done===5?'✓ 已完成':`✓ 確認單 ${done}/5`;btn.style.cssText='font-size:11px;padding:6px 8px;white-space:nowrap';btn.onclick=e=>{e.stopPropagation();openChecklist(classId,i);};checkTd.appendChild(btn);if(cells[5])row.insertBefore(phoneTd,cells[5]);else row.appendChild(phoneTd);const cells2=row.querySelectorAll('td');if(cells2[6])row.insertBefore(checkTd,cells2[6]);else row.appendChild(checkTd);});
  }
  ...
  // 全站 modal：點灰色空白背景關閉；刪除學生後關閉相關子視窗並回到班級清單。
  ...
  const oldDelete=window.deleteStudent;if(typeof oldDelete==='function')window.deleteStudent=function(){const args=arguments,r=oldDelete.apply(this,args);setTimeout(()=>{document.querySelectorAll('#studentNotebookModal.open,#studentChecklistModal').forEach(m=>closeModal(m));const detail=document.getElementById('classDetailModal');if(detail)detail.classList.remove('open');},0);return r;};
